"""Asynchronous Guardrails SDK client."""
...
from .client import GuardrailsConfig
...
class AsyncGuardrailsClient:
    """
    Guardrails SDK client for Python (asynchronous).
    ...
    def __init__(
        ...
        """
        Initialize the async Guardrails client.
    ...
    async def evaluate(
        ...
        """
        Evaluate text against policies (async).
    ...
    async def wrap(
        ...
        """
        Wrap an LLM call with guardrails (async).
        ...
        Args:
            ...
            call: Async function that makes the actual LLM call
            user_id: Optional user identifier
            session_id: Optional session identifier
        ...
        # Evaluate input
        input_result = await self.evaluate(
            text=input_text,
            scope="llm.input",
            user_id=user_id,
            session_id=session_id,
        ...
        # Evaluate output
        output_result = await self.evaluate(
            text=output_text,
            scope="llm.output",
            user_id=user_id,
            session_id=session_id,
